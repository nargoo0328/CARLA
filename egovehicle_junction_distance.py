...
def find_road(road_points,pos,acceleration):
    ...
    for w in road_points:
        ...
        if abs(w_pos.x-pos.x)<=2 and abs(w_pos.y-pos.y)<=2 and abs(w_pos.z-pos.z)<=2:
            next_node=w.next(1)[0].transform.location
            next_node.x-=w_pos.x
            next_node.y-=w_pos.y
            next_node.z-=w_pos.z
...
def run_simulation(args, client):
    ...
    try:
        ...
        while True:
            ...
            try:
                print("road : ",this_w.road_id)
            except:
                if this_w=="c":
                    print("can not find road")
                else:
